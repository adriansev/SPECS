...
%if %{with_lua}
%{!?lua_version: %define luaver %(lua -v 2>&1 | cut -f2 -d' ' | cut -f1,2 -d '.')}
...
# Init script/sysconfig for rrdcached
##%{__mkdir} -p %{buildroot}%{_initrddir}
##%{__cp} %{SOURCE3} %{buildroot}%{_initrddir}/rrdcached

##%{__mkdir} -p %{buildroot}%{_sysconfdir}/default
##%{__cp} %{SOURCE4} %{buildroot}%{_sysconfdir}/default/rrdcached
...
/usr/sbin/useradd -s /sbin/nologin -g %rrdcached_user -c %rrdcached_user -d %{_localstatedir}/run/%rrdcached_user %rrdcached_user &>/dev/null || :
