...
%__install -d -m 0755 $RPM_BUILD_ROOT/etc/gweb
%__cp -f apache.conf $RPM_BUILD_ROOT/etc/gweb/

%files
%defattr(-,root,root)
%attr(0755,nobody,nobody)/var/lib/ganglia-web/filters
%dir %attr(0755,apache,apache)/var/lib/ganglia-web/conf
%dir %attr(0755,apache,apache)/var/lib/ganglia-web/dwoo
...
%config(noreplace) /etc/gweb/apache.conf
...
if [ $1 -eq 1 -a $2 -eq 1 ]; then
	if [ ! -e /etc/httpd/conf.d/ganglia-web.conf ] ; then
		ln -s /etc/gweb/apache.conf /etc/httpd/conf.d/ganglia-web.conf
	fi
fi

%triggerun -- httpd
if [ $2 -eq 0 ]; then
	if [ -h /etc/httpd/conf.d/ganglia-web -a "`readlink /etc/httpd/conf/httpd.conf`" = "/etc/gweb/apache.conf" ]; then
